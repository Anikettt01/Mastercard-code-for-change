...
const Signup = () => {
  ...
  const sendEmail= async()=>{
    try {
      ...
      const {data} = await axios.post("http://localhost:8000/send-email",{to:inputValue.email});
        console.log(data);
    } catch (error) {
      ...
      <div className="relative w-full max-w-md bg-blue-500 p-6 rounded-lg shadow-lg">
        ...
        <div className="relative z-10">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <label htmlFor="username" className="block text-sm mt-1 font-medium text-gray-700">Username</label>
            <input
              type="text"
              name="username"
              value={username}
